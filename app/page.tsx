        ...
        <header className="border-b border-white/[0.08]">
          <div className="mx-auto flex h-[76px] max-w-[1500px] items-center justify-between px-5 sm:px-8 lg:px-12">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-display text-[1.35rem] leading-none tracking-[-0.02em] text-white">
                  A Tuner
                </p>
                <p className="mt-1 text-[0.66rem] font-semibold uppercase tracking-[0.24em] text-brass">
                  Santour tuner
                </p>
              </div>
            </div>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="rounded-full text-muted-foreground hover:bg-white/[0.07] hover:text-white"
                ...
                <TooltipContent side="bottom">
                  Play one course at a time and let the tone settle.
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
            ...
            <div className="relative z-10 flex items-start justify-between gap-4">
              <div>
                <div className="eyebrow">
                  <Waves className="size-3.5" /> Live pitch
                </div>
                <h1 className="mt-2 font-display text-2xl tracking-tight text-white sm:text-3xl">
                  Tune by ear. Confirm by precision.
                </h1>
              </div>
              <div className="hidden rounded-full border border-white/[0.08] bg-black/15 px-3 py-1.5 font-mono text-xs text-white/45 sm:block">
                ± 50 cents
              </div>
            </div>
            ...
            <div className="relative z-10 mx-auto mt-10 flex max-w-3xl flex-col items-center sm:mt-14">
              <div className={`pitch-orbit ${inTune ? "is-tuned" : ""}`}>
                <div className="pitch-orbit-inner">
                  <p className="text-xs font-semibold uppercase tracking-[0.26em] text-muted-foreground">
                    {tuner.isListening ? "Detected note" : "Waiting"}
                  </p>
                  <div className="mt-1 flex items-start justify-center text-white">
                    <span className="font-display text-[6.5rem] leading-none tracking-[-0.07em] sm:text-[8rem]">
                      {noteLetter}
                    </span>
                    <span className="mt-3 font-display text-3xl text-brass sm:text-4xl">
                      {noteSuffix}
                    </span>
                  </div>
              ...
              <div className="mt-10 w-full max-w-[680px]">
                <div className="relative h-20">
                  ...
                  <div
                    className={`meter-needle ${
                      tuner.pitch ? "opacity-100" : "opacity-0"
                    }`}
                    style={{
                ...
                <div className="mt-2 flex justify-between font-mono text-[0.68rem] uppercase tracking-[0.16em] text-white/30">
                  <span>Flat</span>
                  <span className={inTune ? "text-brass" : "text-white/45"}>
                    {tuner.pitch
                      ? `${rawCents > 0 ? "+" : ""}${rawCents.toFixed(1)} ct`
                      : "0 ct"}
                  </span>
                  <span>Sharp</span>
                </div>
              ...
              <div className="mt-9 flex min-h-12 items-center justify-center">
                <div className={`status-pill ${inTune ? "is-tuned" : ""}`}>
              ...
              {tuner.error && (
                <p
                  role="alert"
                  className="mt-4 max-w-md text-center text-sm leading-relaxed text-red-300/90"
                >
                  {tuner.error}
                </p>
              )}
              <Button
                ...
                className="mt-7 h-13 rounded-full bg-brass px-7 text-[0.92rem] font-semibold text-[#15120d] shadow-[0_12px_40px_rgba(207,168,92,0.18)] hover:bg-[#dfbd75] focus-visible:ring-brass/40"
              >
                {tuner.isListening ? (
                ...
                {tuner.isListening ? "Stop listening" : "Start listening"}
              </Button>
              <p className="mt-3 text-center text-xs text-white/30">
                Best results: pluck a single course in a quiet room.
              </p>
            </div>
          </section>
            ...
            <section className="control-card rounded-[1.5rem] border border-white/[0.08] p-5 sm:p-6">
              ...
              <div className="mt-6 flex items-end justify-between">
                <div>
                  ...
                  <p className="mt-1 font-mono text-2xl text-white">
                    A4 = {concertPitch}{" "}
                    <span className="text-sm text-white/35">Hz</span>
                  </p>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => setConcertPitch(440)}
                  disabled={concertPitch === 440}
                  className="h-8 rounded-full px-3 text-xs text-white/40 hover:bg-white/[0.06] hover:text-white"
                >
                  Reset
                </Button>
              </div>
              <Slider
                className="mt-5 [&_[data-slot=slider-track]]:bg-white/10 [&_[data-slot=slider-range]]:bg-brass [&_[data-slot=slider-thumb]]:border-brass [&_[data-slot=slider-thumb]]:bg-[#efe2c1]"
                min={430}
                max={450}
                step={1}
                value={[concertPitch]}
                onValueChange={(value) => setConcertPitch(value[0])}
                aria-label="Concert pitch"
              />
              <div className="mt-2 flex justify-between font-mono text-[0.65rem] text-white/25">
                <span>430</span>
                <span>440</span>
                <span>450</span>
              </div>
              <div className="mt-6 grid grid-cols-2 gap-2">
                <div className="mini-stat">
                  <Volume2 className="size-4 text-white/35" />
                  <div>
                    <p className="text-[0.65rem] uppercase tracking-wider text-white/30">
                      Signal
                    </p>
                ...
                <div className="mini-stat">
                  <Gauge className="size-4 text-white/35" />
                  <div>
                    <p className="text-[0.65rem] uppercase tracking-wider text-white/30">
                      Precision
                    </p>
                    <p className="mt-0.5 text-sm text-white/75">± 1 cent</p>
                  </div>
                </div>
              </div>
            </section>
            <div className="hidden items-center gap-3 px-2 text-xs leading-relaxed text-white/30 lg:flex">
              <Headphones className="size-4 shrink-0" />
              Headphones can help prevent feedback while tuning.
            </div>
          </aside>
        </div>
      </main>
